...
function Load_Ewano_Gateway() {
    ...
    add_filter('woocommerce_payment_gateways', 'Woocommerce_Add_Ewano_Gateway' );
    ...
    class WC_Ewano extends WC_Payment_Gateway {
        ...
        public function __construct(){
            ...
            add_action('woocommerce_receipt_'.$this->id.'', array($this, 'Send_to_Ewano_Gateway_By_HANNANStd'));
            add_action('woocommerce_api_'.strtolower(get_class($this)).'', array($this, 'Return_from_Ewano_Gateway_By_HANNANStd') );
        ...
        public function Send_to_Ewano_Gateway_By_HANNANStd($order_id){
            global $woocommerce;
            $woocommerce->session->order_id_bankmellat = $order_id;
            $order = new WC_Order( $order_id );
            $currency = $order->get_order_currency();
            $currency = apply_filters( 'WC_Ewano_Currency', $currency, $order_id );
            $action = $this->author;
            do_action( 'WC_Gateway_Payment_Actions', $action );
            ...
            $form = apply_filters( 'WC_Ewano_Form', $form, $order_id, $woocommerce );

            do_action( 'WC_Ewano_Gateway_Before_Form', $order_id, $woocommerce );
            echo $form;
            do_action( 'WC_Ewano_Gateway_After_Form', $order_id, $woocommerce );
            ...
            if ( isset($_POST["bankmellat_submit"]) ) {
                ...
                do_action( 'WC_Ewano_Gateway_Payment', $order_id );
                ...
                $callBackUrl = add_query_arg( 'wc_order', $order_id , WC()->api_request_url('WC_Ewano') );
                ...
                if ($client->fault) {
                    ...
                    if ($err) {
                        ...
                        if ($ResCode == "0") {
                            ...
                            $Notice = apply_filters( 'WC_Ewano_Before_Send_to_Gateway_Notice', $Notice, $order_id );
                            if ( $Notice )
                                wc_add_notice( $Notice , 'success' );
                            do_action( 'WC_Ewano_Before_Send_to_Gateway', $order_id );
                ...
                if ($is_error == 'yes') {
                    ...
                    $Note = sprintf( __( 'خطا در هنگام ارسال به بانک : %s', 'woocommerce'), $this->Fault_Ewano($fault) );
                    $Note = apply_filters( 'WC_Ewano_Send_to_Gateway_Failed_Note', $Note, $order_id, $fault );
                    $order->add_order_note( $Note );


                    $Notice = sprintf( __( 'در هنگام اتصال به بانک خطای زیر رخ داده است : <br/>%s', 'woocommerce'), $this->Fault_Ewano($fault) );
                    $Notice = apply_filters( 'WC_Ewano_Send_to_Gateway_Failed_Notice', $Notice, $order_id, $fault );
                    if ( $Notice )
                        wc_add_notice( $Notice , 'error' );

                    do_action( 'WC_Ewano_Send_to_Gateway_Failed', $order_id, $fault );
                }
            }
        }

        public function Return_from_Ewano_Gateway_By_HANNANStd(){
            ...
            if ( $order_id ) {
                ...
                if($order->status !='completed'){
                    ...
                    if ($_POST['SaleOrderId'])
                        update_post_meta( $order_id, 'WC_Ewano_settleSaleOrderId', $_POST['SaleOrderId'] );
                    if ($_POST['SaleReferenceId'])
                        update_post_meta( $order_id, 'WC_Ewano_settleSaleReferenceId', $_POST['SaleReferenceId'] );
                    //------
                    ...
                    if ( $status == 'completed') {
                        ...
                        $Note = apply_filters( 'WC_Ewano_Return_from_Gateway_Success_Note', $Note, $order_id, $transaction_id, $SaleOrderId );
                        if ($Note)
                        ...
                        $Notice = apply_filters('WC_Ewano_Return_from_Gateway_Success_Notice', $Notice, $order_id, $transaction_id, $SaleOrderId );
                        if ($Notice)
                            wc_add_notice( $Notice , 'success' );


                        do_action( 'WC_Ewano_Return_from_Gateway_Success', $order_id, $transaction_id , $SaleOrderId);
                    ...
                    elseif ( $status == 'cancelled') {
                        ...
                        $Note = apply_filters( 'WC_Ewano_Return_from_Gateway_Cancelled_Note', $Note, $order_id, $transaction_id , $SaleOrderId);
                        if ( $Note )
                            $order->add_order_note( $Note, 1 );
                        ...
                        $Notice = apply_filters( 'WC_Ewano_Return_from_Gateway_Cancelled_Notice', $Notice, $order_id, $transaction_id, $SaleOrderId );
                        if ($Notice)
                            wc_add_notice( $Notice , 'error' );

                        do_action( 'WC_Ewano_Return_from_Gateway_Cancelled', $order_id, $transaction_id, $SaleOrderId );
                        ...
                        $Note = sprintf( __( 'خطا در هنگام بازگشت از بانک : %s %s %s', 'woocommerce'), $this->Fault_Ewano($fault), $tr_id , $sale_order_id );
                        $Note = apply_filters( 'WC_Ewano_Return_from_Gateway_Failed_Note', $Note, $order_id, $transaction_id, $SaleOrderId, $fault );
                        if ($Note)
                            $order->add_order_note( $Note , 1 );
                        ...
                        $Notice = str_replace("{fault}",$this->Fault_Ewano($fault),$Notice);
                        $Notice = apply_filters( 'WC_Ewano_Return_from_Gateway_Failed_Notice', $Notice, $order_id, $transaction_id, $SaleOrderId, $fault );
                        if ($Notice)
                            wc_add_notice( $Notice , 'error' );
                        ...
                        do_action( 'WC_Ewano_Return_from_Gateway_Failed', $order_id, $transaction_id, $SaleOrderId, $fault );
                    ...
                    $SaleOrderId = get_post_meta( $order_id, 'WC_Ewano_settleSaleOrderId', true );
                    ...
                    $Notice = apply_filters( 'WC_Ewano_Return_from_Gateway_ReSuccess_Notice', $Notice, $order_id, $transaction_id, $SaleOrderId );
                    if ($Notice)
                        wc_add_notice( $Notice , 'success' );


                    do_action( 'WC_Ewano_Return_from_Gateway_ReSuccess', $order_id, $transaction_id, $SaleOrderId );
                ...
                $Notice = apply_filters( 'WC_Ewano_Return_from_Gateway_No_Order_ID_Notice', $Notice, $order_id, $fault );
                if ($Notice)
                    wc_add_notice( $Notice , 'error' );

                do_action( 'WC_Ewano_Return_from_Gateway_No_Order_ID', $order_id, $transaction_id, $fault );
        ...
        private static function Fault_Ewano($err_code){
...
add_action('plugins_loaded', 'Load_Ewano_Gateway', 0);
